...
class CashedReports extends StatefulWidget

{



  static  const id= "CashedReports";

  @override
  _CashedReportsState createState() => _CashedReportsState();
  var sellerdata ;

  CashedReports({this.sellerdata});
}

class _CashedReportsState extends State<CashedReports> {
  ...
  Widget  AllTime() {
    ...
    var adminpercent;
    firestore.collection("SellerReports").doc(widget.sellerdata) .collection("CashReports").get().then((snapShot) async {
 ...
 x = timetotal;
        ...
        firestore.collection("SellerReports").where('SellerID', isEqualTo: widget.sellerdata).get().then((snapShot) async {
          snapShot.documents.forEach((doc) {
             adminpercent =  doc.data()['adminpercent'];
          });
...
print(adminpercentage);
  ...
  Widget loadsellers() {
    shopworth= x-(x*adminpercentage/100);
    adminworth = x*adminpercentage/100;
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: StreamBuilder<QuerySnapshot>(
            stream: store.loadselelrscashAlltime(widget.sellerdata),
...
,
  ...
  ThisMonth() {
    ...
    var monthtotal = 0.0;
    firestore.collection("SellerReports").doc(widget.sellerdata) .collection("CashReports")
  ...
  Widget loadsellersThisMonth() {
    return Padding(
      ...
      child: Container(
        child: StreamBuilder<QuerySnapshot>(
            stream: store.loadsellerthismonthcashed(widget.sellerdata),
            // ignore: missing_return
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                if (snapshot.data.docs.length == 0) {
  ...
  Widget loadsellersThisweek() {
    return Padding(
      ...
      child: Container(
        child: StreamBuilder<QuerySnapshot>(
            stream: store.loadsellerthisweekcashed(widget.sellerdata),
            // ignore: missing_return
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                if (snapshot.data.docs.length == 0) {
      ...
      buttons: [
        DialogButton(
            ...
            onPressed: () {
              ...
              adjustpercentage(widget.sellerdata, _percentagecontroller);
  ...
  void adjustpercentage(sellerdata, TextEditingController percentagecontroller, ) {
    firestore.collection("SellerReports").doc(sellerdata).update({
      'adminpercent': double.parse(_percentagecontroller.text)
    });
    setState(() {
adminpercentage;
    });
